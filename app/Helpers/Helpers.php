...
if (!function_exists('getComplaintStatusBadge')) {
    ...
    function getComplaintStatusBadge($complaint)
    {
        ...
        $assignedAt = Carbon::parse($complaint->assigned_at);

        if ($status == 0 && $department > 0) {
            if ($assignedAt->lt(now()->subDays(2))) {
        ...
        }  elseif ($status == 2) { // assuming 2 = reopened
            $badge .= '<span class="badge bg-warning"><i class="fas fa-redo"></i> Reopened</span>';
        }else {
